...
- **Modified:**
  ...
  - `src/components/project/projects-dashboard.tsx`: Fixed invisible workspace title in Workspace Hero banner in Light Mode; equalized card heights (`min-h-[260px]`); resolved `lofi-panel` class override on `QuickCreateBoardBlueprintCard` so dashed blueprint border renders crisply; upgraded `WorkspaceBoardCard` and `ProjectCard` progress bar tracks (`bg-stone-200 dark:bg-white/10`), column preview chips, and stat pills for crisp contrast in Light Mode; redesigned `Search workspaces...` and `Search boards...` inputs with standard `h-9.5`, centered magnifying glass icons, rounded clear `X` buttons, and crisp Light Mode text/borders; upgraded category filter tabs (`All`, `Starred`, `Work`, `Diary`) for high-contrast Light Mode.
  - `src/components/kanban/board.tsx`: Redesigned `Search cards...` input with standard `h-9` rounded-xl styling, centered icon, and clear button; upgraded toolbar filter buttons (`Today`, `My Tasks`, `Assignee`, `Clear filters`, `Undo`) with high-contrast Light Mode colors and dark mode fallbacks; upgraded `DragCardOverlay` title for Light Mode ink contrast.
  - `src/components/kanban/board-settings-modal.tsx`: Fixed member search input height from invalid `h-8.5` to `h-9.5` with centered icon and clear button.
  - `src/lib/theme/card-colors.ts`: Upgraded all 10 card color options (`DEFAULT`, `LAVENDER`, `CYAN`, `AMBER`, `ROSE`, `EMERALD`, `VIOLET`, `BLUE`, `WINE`, `SLATE`) with explicit dual-theme classes: pure crisp paper surfaces (`bg-white` / subtle tinted paper) with elegant colored borders in Light Mode, and dusky translucent tones in Dark Mode.
  - `src/components/ui/avatar.tsx` & `src/components/kanban/assignee-avatar.tsx`: Upgraded `AVATAR_COLORS` for high-contrast light mode text and backgrounds; replaced dark `ring-ink-950` with `ring-white dark:ring-ink-950` to eliminate dark halos around avatars on white cards; updated assignee name text contrast.
