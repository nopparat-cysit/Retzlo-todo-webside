export const retroStickerOptions = [
...
const retroStickerSrcSet: Set<string> = new Set(retroStickerOptions.map((sticker) => sticker.src));

export function isRetroStickerPath(value: string) {
  return retroStickerSrcSet.has(value);
}
