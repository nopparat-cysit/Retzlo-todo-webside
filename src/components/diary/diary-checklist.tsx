...
export function DiaryChecklistEditor({
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-white/10 bg-white/[0.025] p-3">
      ...
      <div className="mb-3 rounded-lg border border-white/10 bg-ink-950/30 p-3">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1 scrollbar-soft">
        {items.length === 0 ? (
          <div className="rounded-md border border-dashed border-white/10 p-4 text-center text-xs text-stone-500">
            No checklist items yet.
          </div>
        ) : (
          items.map((item, index) => (
            <div key={item.id} className="rounded-lg border border-white/10 bg-ink-950/35 p-3">
              <div className="mb-3 flex items-center justify-between gap-2">
                <span className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-500">Item {index + 1}</span>
                <button
              ...
              <div className="grid gap-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <p className="text-[11px] leading-5 text-stone-500 sm:col-span-2">
                    Starts from the diary start date.
                  </p>
                </div>
              </div>
            </div>
...
export function DiaryChecklistPreview({ canManage, onChange, selectedDate, value }: DiaryChecklistPreviewProps) {
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-xl border border-white/10 bg-ink-950/25">
      <div className="border-b border-white/10 bg-white/[0.035] px-3 py-2.5">
        <div className="flex items-center justify-between gap-3">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-400">Checklist</p>
      ...
      <div className="max-h-72 space-y-1.5 overflow-y-auto p-2 scrollbar-soft">
        {sorted.map((item) => {
          ...
          return (
            <label
              key={item.id}
              className={cn(
                "flex w-full items-start gap-3 rounded-lg border border-white/8 bg-white/[0.035] p-3 text-sm transition hover:border-dusk-lavender/20 hover:bg-white/[0.055]",
                !isDue && "opacity-45"
              )}
            >
              <button
              ...
              <span className="min-w-0 flex-1">
                <span className={cn("block truncate font-medium text-stone-200", isCompleted && "line-through opacity-70")}>
                  {item.label}
                </span>
                ...
                <span className="mt-1 flex flex-wrap gap-1.5 text-[10px] text-stone-500">
                  <span className="rounded-full bg-white/[0.045] px-1.5 py-0.5">Every {item.intervalDays}d</span>
                  {item.dueTime ? <span className="rounded-full bg-white/[0.045] px-1.5 py-0.5">{item.dueTime}</span> : null}
                  {!isDue ? <span className="rounded-full bg-white/[0.045] px-1.5 py-0.5">Not due</span> : null}
                </span>
              </span>
            </label>
          );
